...
const Navbar = () => {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 py-1 transition duration-300 ease-in-out ${
    ...
    >
      <nav className="container max-w-7xl mx-auto flex justify-between items-center py-2 px-4">
        ...
        <div className="flex-col md:flex-row items-center md:space-x-8 gap-5 hidden md:flex text-lg">
          <NavLink
            ...
            className={({ isActive }) =>
              isActive
                ? "text-red-400 font-medium border-b-2"
                : "hover:text-red-400"
            }
          >
            Home
          ...
          <NavLink to="/cart" className="md:block cursor-pointer relative">
            ...
            {cartItem.length > 0 && (
              <sup className="absolute -top-2.5 -right-2.5 bg-red-400 text-white w-4.5 h-4.5 rounded-2xl flex items-center justify-center text-xs">
                {cartItem.length}
              </sup>
            )}
          </NavLink>
        ...
        {/* Mobile icon */}
        <button
          className="flex text-2xl md:hidden lg:hidden transition-transform duration-500"
      ...
      {isMobileMenuOpen && (
        <div
          className={`md:hidden lg:hidden bg-white shadow-md absolute top-full left-0 w-full h-screen z-40 transition-all duration-800 ease-in-out transform ${
            isMobileMenuOpen
